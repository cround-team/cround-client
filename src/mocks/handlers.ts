...
export const handlers = [
  ...
  rest.get("http://54.180.159.108/api/creators", (req, res, ctx) => {
    return res(
      ctx.delay(1000),
      ctx.status(200),
      ctx.json({
  ...
  rest.get("http://54.180.159.108/api/shorts/populars", (req, res, ctx) => {
    return res(
      ctx.delay(1000),
      ctx.status(200),
      ctx.json({
        popularVisitShortForms: [
          {
            shortsId: 0,
            title:
              "ASMR 팁 방출 및 ASMR 분야의 모든것을 알려드리고자 합니다. ASMR 팁 방출 및 ASMR 분야의 모든것을 알려드리고자 합니다. ASMR 팁 방출 및 ASMR 분야의 모든것을 알려드리고자 합니다.",
